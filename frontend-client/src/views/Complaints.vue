    ...
    <div v-if="showReplyModal" class="modal-mask" @click.self="showReplyModal = false">
      <div class="modal reply-modal">
        ...
        <div v-if="replyList.length > 0" class="reply-history">
          <label>历史回复</label>
          <div v-for="reply in replyList" :key="reply.id" class="reply-history-item">
          <div class="reply-history-header">
            <span class="reply-role" :class="reply.ownerId ? 'role-owner' : 'role-admin'">{{ replyLabel(reply) }}</span>
            <span class="reply-read" :class="reply.ownerRead ? 'read' : 'unread'">{{ reply.ownerRead ? '✓ 已读' : '● 未读' }}</span>
            <span class="reply-time">{{ formatDate(reply.createdAt) }}</span>
          </div>
          <div class="reply-history-content">{{ reply.replyContent }}</div>
        </div>
...
<script setup>
import { onMounted, onUnmounted, reactive, ref, computed, watch, onActivated } from 'vue'
...
async function markAllOwnerRead() {
  const ownerId = userStore.user?.ownerId
  if (ownerId) {
    try {
      await request.post(`/complaints/owner/${ownerId}/mark-all-read`)
    } catch (e) {
...
watch(
  () => route.fullPath,
  () => {
    console.log('🔄 Complaints 路由变化，重新加载数据')
    loadData()
  },
  { immediate: false }
)
onActivated(() => {
  console.log('🔄 Complaints 被激活，重新加载数据')
  loadData()
})
...
.reply-modal .reply-item {
  flex-shrink: 0;
}

.reply-modal .reply-history {
  flex: 1;
  overflow-y: auto;
  min-height: 0;
  margin-bottom: 16px;
